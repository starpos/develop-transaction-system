...
In-memory DBMS 向けの並行実行制御プロトロルについて紹介します。


=== シングルスレッド vs マルチスレッド

リファレンス設計の基本ではシングルスレッドで並行実行制御ナシという設計でした。
...
例えば、使っているプログラミング言語が非同期処理(async/await とか、コルーチンなど)を
...
mutex と condition_variable を用いて作る方法が考えられます。
...
一番単純なのは nowait と呼ばれる deadlock prevention 手法です。
