 ...
 * NVRAM (3D Xpoint が発売にこぎつけましたが、それがメモリ階層の中で不可欠な役割を担うようになるかまだ五分五分と私は見ています。これは 2019 年の見解で、2021 年現在、雲行きが怪しいです。。2022 年現在、Optane のビジネスが終わることがほぼ確実となりました。)
 * CXL (Compute Express Link) によるキャッシュコヒーレンシを保ったメモリアクセスのエコシステム化。
