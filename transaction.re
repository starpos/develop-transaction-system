...
私はこれをトランザクションシステムだと呼ぶのには抵抗があります．
...
Begin, commit や abort 操作は DML とは区別され，トランザクション制御文と呼ばれるようです．
...
Commit 操作を実行しても，その返事が返ってくるまでは成功したかどうか分かりません．
...
それ本当に「トランザクション実行できるんですか？？？」という疑問は常に持つようにしましょう．
...
One-shot トランザクションのみ想定するならアプリケーションとのやりとりの遅延を隠蔽できるので，
...
高速さやスケーラビリティを求めた一部のプロダクトで one-shot トランザクションに特化したものもあります．
...
組込み DBMS であるが故にアプリケーションと DBMS 間でのネットワーク等を介したプロトコルが不要です．
組込みでない DBMS で one-shot トランザクションをサポートしようと考えたとき，
ストアドプロシージャを記述する専用言語と実行ランタイムを用意する方法もあり得るでしょうし，
もう少し楽な方法としてプラグインで実現する方法もあるでしょうが，
もっともっとお手軽に作るにはストアドプロシージャは DBMS と同じコードで定義してしまい，
一緒にコンパイルしてしまうのが良いでしょう．


大雑把に言うと，DBMS のアーキテクチャは大きく二種類あって，以下のどちらかです．

 1. 別プロセス: サーバープロセス形式で実装されネットワーク経由で操作およびデータのやりとりをする
 2. 同一プロセス: ライブラリ形式で実装され，関数呼び出し経由で操作およびデータのやりとりをする

1. よりも 2. の方が設計/実装が大変であることは想像がつきますでしょうか？
テストやデバッグ，ベンチマークのみを差し当っての目的にするのであれば，
ライブラリとして作るよりも，
DBMS とトランザクションロジックを一緒のソースコードとして開発してしまい，
一緒にコンパイル/ビルド/リンク等してしまうのがもっと単純です．
