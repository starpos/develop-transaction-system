...
//footnote[cpp-order][C++ などでは、@<tt>{!(a < b) && !(b < a)} ならば @<tt>{a == b} とする考え方もあります。これは total order を前提としています。]
...
Record 型に含まれる複数の column 型には順序がないと考えることも可能ですが、
...
Key は record を入力とする関数もしくはその値と考えることができます。
...
例えば table の key と言ったときは、table から作られる何らかの key 型を指すものとし、
ある record の key と言ったときは、何らかの key 値を指すものとします。

ひとつの table について column の選び方やその順序によって key 型は複数存在します。
一般に、record と key の対応は n:1 の関係があります(ここでの n は一般に複数という意味)。
Key が record を区別するのに十分な情報を持っていると見做せるとき、
...
実は unique key というのは、現状そうなっているということではなくスキーマに与える制約のことなのです。
...
一般的なデータベース設計では、意図的に uniqueness を担保するために、何の意味もない数字を各 record に割り当てて
それを unique key として record を区別できるようにします。ここでは @<tt>{Id} key がそれにあたります。

//note{
今の時代なら皆さんにはマイナンバーが割り当てられていますね。
個人にひとつ unique な番号を割り当てられることを仕組みとして担保しているので、
国や自治体が国民/住民のデータベースを管理する場合は、システム毎に @<tt>{Id} を割り振るよりも、
unique key としてマイナンバーを使った方が良いかも知れませんね。
...
複数の column 値から構成される key 値の順序を考える場合は辞書順で考えます。
...
//footnote[secondary-index][Secondary index と呼ばれるものは、本質的な仕組みに少し関係あるかも知れません。]
