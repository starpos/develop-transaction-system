...
Hadoop や Spark などは、細かい単位でのデータ操作が苦手で、とりわけトラ
ンザクション処理には対応していない代わりに、SQL SELECT 文の実行、つま
り、データを大量に加工、集計する用途に特化しています。また、ひとつのホ
ストでは保持できない程の大量のデータを扱えるように、複数のホストをまた
がって動作する分散システムです。etcd は、設定情報を分散システムで安全
に共有する目的で使われることが多いデータベースシステムですが、
Key-value store に分類されます。Key-value store は、主に key を指定し
て、value を検索、操作するという、SQL よりも単純なインターフェースを持っ
ています。Key-value store に分類される memcached や Redis は、データの
永続化をある程度犠牲にしてでも、高速に処理したいデータを扱う目的で使わ
れることが多いようです。
...
本書では、上記で紹介した様々なデータベースシステムの一部が持っている機
...
たくさんいるからです。また、トランザクションを伴うアプリケーションを設
計実装する人達は、ひとまとめにしたい複数のデータベース操作をひとつのト
ランザクションにまとめるだけで済むことが多いと思います。それほどトラン
ザクションという概念のもたらす抽象化は良く出来ており、使う側にとっては
便利な道具だと思います。トランザクションは万能に見えるかも知れませんが、
...
造)、効率的に更新データの永続化機構 (Write-ahead logging, WAL や
checkpointing)、およびトランザクションを同時にたくさん実行するための並
行実行制御(Concurrency Control) です。
...
本当に複数の CPU コアを使って複数のスレッド/プロセスでトランザクション
を並列に実行する場合、複数スレッド/プロセスからひとつのデータ構造にア
クセスするので、並列プログラミングと言われている技術が必要になります。
具体的には lock や latch などデータの適切な排他を行う仕組みが、場合に
よっては、lock-free だとか mutex-free と呼ばれているような技術もです。

皆さん次第ではありますが、これらの技術的に難易度が高めの能力を前提とし
てしまうのは、本書の目的に必ずしも沿わないと思います。
...
基本的なアルゴリズムとデータ構造について、Tree Map 構造と hash table
構造のインデクスとして使ったときの特性の違いだとか、search と sort ア
ルゴリズムだとか。分からなければ、必要に応じて学べば良いです。
