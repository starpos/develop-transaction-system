...
その名の通り，log を (data より) 先に書く方式を意味します．
...
WAL は ACID 特性の，A と D を実現するための仕組みです．
トランザクションが複数の操作からなり，それらの操作を crash が発生し得る世界で atomic に実行したのと同じ結果を得るためです．
Crash が発生したときは，データベース本体のファイルが中途半端な状態になってしまうことは避けられません．
というのも atomic にしたい複数操作が実際には atomic に永続化を伴って実行できないからです．
...
//footnote[footnote_reads_from][Reads-from 関係ともいいます．B reads from A とか B reads x written by A といったりします．]
