...
素朴には、データベースの全データを掃き出せば checkpointing できますが、掃き出されたデータは
一貫性のある snapshot ではなく、中途半端なデータベース状態です。
詳細はここでは述べませんが、これを log と組み合わせて一貫性のある snapshot を作り出す必要があります。
...
Checkointing の良さとは、差分を取り出すためのオーバーヘッドが小さいことと、
